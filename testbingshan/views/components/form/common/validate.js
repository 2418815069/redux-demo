...
const requiredFields = [
  'testCenter', 'caseNumber',
  'groupNumber', 'aliasTestCenter',
];
...
const submitAndValidate = (values) => {
  ...
  requiredFields.forEach((field) => {
    if (!values[field] && field === 'caseNumber') {
      errors[field] = '请输入病例筛选编号!';
    } else if (!values[field] && field === 'testCenter') {
      errors[field] = '请选择试验中心!';
    } else if (!values[field] && field === 'groupNumber') {
      errors[field] = '请输入入组编号!';
    } else if (!values[field] && field === 'aliasTestCenter') {
      errors[field] = '请选择试验中心编号!';
    }
  });
